...
export default function BeritaIndex({ items, filters }: Props) {
    ...
    const doSearch = useCallback((keyword: string) => {
        router.get('/staf/berita', { search: keyword.trim() || undefined }, { preserveState: true, preserveScroll: true, replace: true });
    }, []);
    ...
    const openCreate = () => router.visit('/staf/berita/create');
    const openEdit = (item: Item) => router.visit(`/staf/berita/${item.id}/edit`);
    ...
    const handleTogglePublish = (item: Item) => {
        router.post(`/staf/berita/${item.id}/toggle-publish`, {}, { preserveScroll: true, onError: () => showError('Gagal mengubah status publikasi.') });
    };

    const handleDelete = () => {
        if (!deletingItem) return;
        router.delete(`/staf/berita/${deletingItem.id}`, { preserveScroll: true, onSuccess: () => { setIsDeleteOpen(false); setDeletingItem(null); }, onError: () => showError('Gagal menghapus berita.') });
    };

    return (
        <StafLayout title="Berita" breadcrumbs={[{ label: 'Dashboard', href: '/staf' }, { label: 'Berita' }]}>
            <div className="space-y-6">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        <h1 className="text-2xl font-bold">Berita</h1>
                        <p className="text-base-content/60 mt-1">Kelola berita Kelurahan Ardipura</p>
                    </div>
                    <Button color="primary" onClick={openCreate}>+ Tambah Berita</Button>
                </div>
                    ...
                    <div className="card-body p-4">
                        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                            <label className="input flex-1 min-w-0 flex items-center gap-2">
                                ...
                                <input type="text" className="grow bg-transparent border-none outline-none focus:outline-none focus:ring-0 p-0" placeholder="Cari judul berita..." value={search} onChange={(e) => handleSearchChange(e.target.value)} />
                        ...
                        <p className="text-xs text-base-content/50 mt-2">Menampilkan {items.from ?? 0}–{items.to ?? 0} dari {items.total} berita</p>
                    </div>
                </div>
                ...
                <div className="card bg-base-100 shadow overflow-x-auto">
                    <Table className="table-sm">
                        ...
                        <TableBody>
                            {items.data.length === 0 ? (
                                <TableRow><TableCell colSpan={6} className="text-center py-12 text-base-content/50">Belum ada berita.</TableCell></TableRow>
                            ) : items.data.map((item, i) => (
                                <TableRow key={item.id} className="hover">
                                    <TableCell>{(items.from ?? 1) + i}</TableCell>
                                    <TableCell>
                                        <div className="font-medium">{item.judul}</div>
                                        {item.ringkasan && <div className="text-xs text-base-content/50 line-clamp-1">{item.ringkasan}</div>}
                                    </TableCell>
                                    ...
                                    <TableCell className="text-center">
                                        <div className="flex justify-center gap-1">
                                            ...
                                            <Tooltip tip="Edit"><Button size="sm" color="warning" variant="soft" circle onClick={() => openEdit(item)} title="Edit Berita">✏️</Button></Tooltip>
                                            <Tooltip tip="Hapus"><Button size="sm" color="error" variant="soft" circle onClick={() => openDelete(item)} title="Hapus Berita">🗑️</Button></Tooltip>
                                        </div>
                                    </TableCell>
                                </TableRow>
                            ))}
            ...
            <Modal open={isDeleteOpen} onClose={() => setIsDeleteOpen(false)}>
                ...
                <ModalBody>
                    <p>Yakin ingin menghapus berita <strong>{deletingItem?.judul}</strong>?</p>
                ...
                <ModalBody className="max-h-[70vh] overflow-y-auto">
                    {detailItem && (
                        <article className="space-y-4">
                            <div>
                                <h4 className="text-sm font-semibold text-primary mb-2">Informasi Berita</h4>
                                <div className="grid grid-cols-2 gap-2 text-sm">
                                    <div><span className="text-base-content/60">Judul:</span> {detailItem.judul}</div>
                                    <div><span className="text-base-content/60">Slug:</span> <span className="font-mono text-xs">{detailItem.slug || '—'}</span></div>
                                    <div><span className="text-base-content/60">Penulis:</span> {detailItem.user?.name || '—'}</div>
                                </div>
                            </div>
                            ...
                            <div><h4 className="text-sm font-semibold text-primary mb-2">Isi Berita</h4><article className="prose max-w-none" dangerouslySetInnerHTML={{ __html: detailItem.isi }} /></div>
                            ...
                            <div className="divider my-2" />
                            <div>
                                <h4 className="text-sm font-semibold text-primary mb-2">Publikasi</h4>
                                <div className="grid grid-cols-2 gap-2 text-sm">
                                    <div><span className="text-base-content/60">Status:</span> {detailItem.is_published ? <Badge color="success" size="sm">Published</Badge> : <Badge color="ghost" size="sm">Draft</Badge>}</div>
                                    <div><span className="text-base-content/60">Tgl Publikasi:</span> {detailItem.published_at ? formatDateTime(detailItem.published_at) : '—'}</div>
                                    <div><span className="text-base-content/60">Dibuat:</span> {formatDateTime(detailItem.created_at)}</div>
                                    <div><span className="text-base-content/60">Diupdate:</span> {formatDateTime(detailItem.updated_at)}</div>
                                </div>
                            </div>
                        </article>
                    )}
