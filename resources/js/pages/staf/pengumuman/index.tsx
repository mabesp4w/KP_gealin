...
export default function PengumumanIndex({ items, filters }: Props) {
    ...
    const doSearch = useCallback((keyword: string) => { router.get('/staf/pengumuman', { search: keyword.trim() || undefined }, { preserveState: true, preserveScroll: true, replace: true }); }, []);
    ...
    const openCreate = () => router.visit('/staf/pengumuman/create');
    const openEdit = (item: Item) => router.visit(`/staf/pengumuman/${item.id}/edit`);
    ...
    const handleTogglePublish = (item: Item) => { router.post(`/staf/pengumuman/${item.id}/toggle-publish`, {}, { preserveScroll: true, onError: () => showError('Gagal mengubah status publikasi.') }); };

    const handleDelete = () => { if (!deletingItem) return; router.delete(`/staf/pengumuman/${deletingItem.id}`, { preserveScroll: true, onSuccess: () => { setIsDeleteOpen(false); setDeletingItem(null); }, onError: () => showError('Gagal menghapus pengumuman.') }); };

    return (
        <StafLayout title="Pengumuman" breadcrumbs={[{ label: 'Dashboard', href: '/staf' }, { label: 'Pengumuman' }]}>
            <div className="space-y-6">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div><h1 className="text-2xl font-bold">Pengumuman</h1><p className="text-base-content/60 mt-1">Kelola pengumuman Kelurahan Ardipura</p></div>
                    <Button color="primary" onClick={openCreate}>+ Tambah Pengumuman</Button>
                </div>
                    ...
                    <div className="card-body p-4">
                        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                            <label className="input flex-1 min-w-0 flex items-center gap-2">
                                ...
                                <input type="text" className="grow bg-transparent border-none outline-none focus:outline-none focus:ring-0 p-0" placeholder="Cari judul pengumuman..." value={search} onChange={(e) => handleSearchChange(e.target.value)} />
                        ...
                        <p className="text-xs text-base-content/50 mt-2">Menampilkan {items.from ?? 0}–{items.to ?? 0} dari {items.total} pengumuman</p>
                    </div>
                </div>
                ...
                <div className="card bg-base-100 shadow overflow-x-auto">
                    <Table className="table-sm">
                        ...
                        <TableBody>
                            {items.data.length === 0 ? (
                                <TableRow><TableCell colSpan={6} className="text-center py-12 text-base-content/50">Belum ada pengumuman.</TableCell></TableRow>
            ...
            <Modal open={isDeleteOpen} onClose={() => setIsDeleteOpen(false)}>
                ...
                <ModalBody><p>Yakin ingin menghapus pengumuman <strong>{deletingItem?.judul}</strong>?</p><p className="text-sm text-error mt-2">Tindakan ini tidak dapat dibatalkan.</p></ModalBody>
                ...
                <ModalBody className="max-h-[70vh] overflow-y-auto">
                    {detailItem && (
                        <article className="space-y-4">
                            ...
                            <div className="divider my-2" /><div><h4 className="text-sm font-semibold text-primary mb-2">Isi Pengumuman</h4><article className="prose max-w-none" dangerouslySetInnerHTML={{ __html: detailItem.isi }} /></div>
